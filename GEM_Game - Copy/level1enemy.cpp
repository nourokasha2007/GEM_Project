...
void Level1Enemy::removeHomingEntry(QTimer* timer, QGraphicsPixmapItem* projectile)
{
    for (int i = homingEntries.size() - 1; i >= 0; --i) {
        if (homingEntries[i].timer == timer && homingEntries[i].projectile == projectile) {
            homingEntries.removeAt(i);
            break;
        }
    }
}

void Level1Enemy::discardActiveHomingProjectiles()
{
    for (HomingEntry& e : homingEntries) {
        if (e.timer) {
            disconnect(e.timer, nullptr, this, nullptr);
            e.timer->stop();
            e.timer->deleteLater();
            e.timer = nullptr;
        }
        if (e.projectile) {
            if (QGraphicsScene* s = e.projectile->scene()) {
                s->removeItem(e.projectile);
            }
            delete e.projectile;
            e.projectile = nullptr;
        }
    }
    homingEntries.clear();
}
...
void Level1Enemy::shootHomingProjectile() {
    ...
    homingEntries.append({homingTimer, projectile});

    connect(homingTimer, &QTimer::timeout, this, [=]() mutable {
        auto stillTracked = [this, homingTimer, projectile]() {
            for (const HomingEntry& e : homingEntries) {
                if (e.timer == homingTimer && e.projectile == projectile) {
                    return true;
                }
            }
            return false;
        ...
        if (player == nullptr) {
            homingTimer->stop();
            disconnect(homingTimer, nullptr, this, nullptr);
            currentScene->removeItem(projectile);
            delete projectile;
            removeHomingEntry(homingTimer, projectile);
